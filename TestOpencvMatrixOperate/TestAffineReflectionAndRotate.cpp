...
TEST(TestAffineReflectionAndRotate, TestSolveFromPoints_SimulatePatternResultDiff)
{
	// ARRANGE
	double stagePoints[4][3] =
	{
	...
	for (int i = 0; i < 4; i++) {
		stagePts.push_back(cv::Mat(1, 3, CV_64FC1, stagePoints[i]));
		imagePoints[i][0] = imagePoints[i][0] + 10;
		imagePoints[i][1] = imagePoints[i][1] + 20;
		imagePts.push_back(cv::Mat(1, 3, CV_64FC1, imagePoints[i]));	
	}
	stagePts = stagePts.t();
	imagePts = imagePts.t();
...
TEST(TestAffineReflectionAndRotate, TestSolveFromPointsWithScale)
{
	// ARRANGE
	double stagePoints[4][3] =
	{
	...
	cv::Mat stagePts, imagePts;
	for (int i = 0; i < 4; i++) {
		stagePoints[i][0] = stagePoints[i][0] * 2;
		stagePoints[i][1] = stagePoints[i][1] * 2;
		stagePts.push_back(cv::Mat(1, 3, CV_64FC1, stagePoints[i]));
		imagePts.push_back(cv::Mat(1, 3, CV_64FC1, imagePoints[i]));	
	}
	stagePts = stagePts.t();
	...
	// ASSERT
	EXPECT_NEAR(*((double*)img2Stg.data + 0),  2.0, 0.001);
	EXPECT_NEAR(*((double*)img2Stg.data + 1),  0.0, 0.001);
	//EXPECT_NEAR(*((double*)img2Stg.data + 2),  0.0, 0.001);
	EXPECT_NEAR(*((double*)img2Stg.data + 3),  0.0, 0.001);
	EXPECT_NEAR(*((double*)img2Stg.data + 4), -2.0, 0.001);
	//EXPECT_NEAR(*((double*)img2Stg.data + 5),  4500.0, 0.001);
	EXPECT_NEAR(*((double*)img2Stg.data + 6),  0.0, 0.001);
	EXPECT_NEAR(*((double*)img2Stg.data + 7),  0.0, 0.001);
	EXPECT_NEAR(*((double*)img2Stg.data + 8),  1.0, 0.001);
...
TEST(TestAffineReflectionAndRotate, TestSolveFromPointsWithScaleUseDelta)
{
	// ARRANGE
	double stagePoints[4][3] =
	{
	...
	for (int i = 1; i < 4; i++) {
		stagePoints[i][0] = stagePoints[i][0] - stagePoints[0][0];
		stagePoints[i][1] = stagePoints[i][1] - stagePoints[0][1];
		stagePts.push_back(cv::Mat(1, 3, CV_64FC1, stagePoints[i]));
		imagePoints[i][0] = imagePoints[i][0] - imagePoints[0][0];
		imagePoints[i][1] = imagePoints[i][1] - imagePoints[0][1];
		imagePts.push_back(cv::Mat(1, 3, CV_64FC1, imagePoints[i]));	
	}
	stagePts = stagePts.t();
	imagePts = imagePts.t();
